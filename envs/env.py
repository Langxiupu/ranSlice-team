...
class LEORANSlicing(gym.Env):
    def __init__(self, file_path, training=True):
        ...
        sats_data = read_sats_bd_rb_csv(self.sats_bd_rbs) # 读取卫星带宽信息
        self.apps_weights_dict = read_app_weight_csv(self.apps_weights)  # 读取卫星权重信息
        ...
        # 然后创建用户对象
        ue_position_dict = read_positions_from_csv(env_config["ue_position"])  # 读取用户位置信息
        # 创建用户终端
        self.UEs = {}
        self.available_UEs = {}
        ...
        if self.flow_gen_type == "on-off":  # 如果流生成方式是"on-off"
            self.flow_gen_params = env_config["flow_gen_method"]["params"]  # 读取流生成方式的参数
        app_flow_types_dict = read_app_flow_csv(self.app_flow_types)  # 读取应用流类型
        ...
        for sat in self.leo_sats:
            ...
            for ue_id, ue in self.UEs.items():
                ...
                print(f"已创建卫星 {sat.ID} 与用户终端 {ue_id} 的信道对象")
        ...
        self._observation_space = {
            str(sat.ID): create_sat_obs_space(sat.ID, len(self.UEs), len(self.leo_sats)-1, app_low, app_high, sat.rbs)
            for sat in self.leo_sats
        }
        self.head_infos = []
        self.autoregressive_maps = []
        self.available = calc_ratio_vec() # 得到的是一个时间跨度为3300秒，每0.1s记录当前地区用户活跃度的vector数组
        #首先是两种类型切片的带宽份额
        head_idx = 0
        self._actions = []
        for sat in self.leo_sats:
    ...
    def reset(self, seed=None):
        ...
        distance_dict = get_info_by_time(self.visibles_t_u, 0, self.common_sats, self.ue_num)  # 获取第0个时隙的用户-卫星距离信息
        # 其次需要对每一个ue进行重置
        for ue_id, ue in self.UEs.items():
            ue.reset()
        self.available_UEs = {}
    ...
    def step(self, actions):
        ...
        for ue_id, ue in self.available_UEs.items():
            ...
            if target_sat_id is not None:
                ...
                if ue.connect_satellite is None or ue.connect_satellite.ID != target_sat_id:
                    # 如果不同，先断开原来的连接
                    ue.disconnect_sat()
                    # 然后连接到新的卫星
                    for sat in self.leo_sats:
            ...
            for sat in self.leo_sats:
                ...
                for ue_id, ue in self.UEs.items():
                    if self.current_slot == 0:
                        ...
                        sat_ue_channel.bandwidth = sat._BW * 1e6  # 转换为Hz
                        sat_ue_channel.scs = sat._scs * 1e3  # 转换为Hz
                        sat_ue_channel.total_subcarriers = sat_ue_channel.num_rb * sat_ue_channel.num_sc_per_rb
                        sat_ue_channel._generate_static_parameters()
                        sat_ue_channel.initialize_channel()
                    for j in range(10): # 遍历10个时隙
                        sat_ue_channel = self.sats_channels[sat.ID][ue_id]
                        snr = sat_ue_channel.get_rb_cnr()
                        if sat_snr_dict[sat.ID].get(ue_id) is None:
                            sat_snr_dict[sat.ID][ue_id] = {}
                            sat_snr_dict_real[sat.ID][ue_id] = {}
                        sat_snr_dict[sat.ID][ue_id][j] = snr
                        sat_snr_dict_real[sat.ID][ue_id][j] = snr
                        if j==9 and self.current_slot == self.slice_slots -1:
                            continue
                        sat_ue_channel.step_channel()
        else:
            # 如果是测试模式，需要从nex_obs中来填充sat_snr_dict
            for sat in self.leo_sats:
                sat_snr_dict[sat.ID] = {}
                sat_snr_dict_real[sat.ID] = {}
                for ue_id, ue in self.UEs.items():
                    ...
                    for j in range(10): # 遍历10个时隙
                        ...
                        if self.next_obs is None:
                            raise ValueError("next_obs is None, cannot extract SNR information.")
                        snr_key = f"{sat.ID}_snr_{j}"
                        snr_array = self.next_obs[snr_key]
                        snr = snr_array[ue_id - 1]  # ue_id从1开始，数组下标从0开始
                        if sat_snr_dict[sat.ID].get(ue_id) is None:
                            sat_snr_dict[sat.ID][ue_id] = {}
                            sat_snr_dict_real[sat.ID][ue_id] = {}
                        sat_snr_dict[sat.ID][ue_id][j] = snr
                        sat_snr_dict_real[sat.ID][ue_id][j] = sat_ue_channel.get_rb_cnr()
                        if j==9 and self.current_slot == self.slice_slots -1:
                            continue
                        sat_ue_channel.step_channel()
        # 让每个ue生成0.1s的数据包
        for ue_id, ue in self.UEs.items():
            # 只有ue是active才会生成
            if ue.active == True:
                ue.generate_flow(self.slice_interval)
        ...
        result = self.mac_scheduler.schedule(self.leo_sats, action_dict, self.UEs, sat_snr_dict)
        if result["success"] == False:
            # 代表下层admm算法调度没有结果，这个时候应该直接从当前episode退出，
            reward = -100.0 * self.ue_num
            done = True
            observation = self.return_zero_obs()
            observation["is_first"] = False
            observation["is_terminal"] = True
            return observation, reward, done, info
        app_results, slice_results = self.mac_scheduler.statistics_summary(self.leo_sats, action_dict, self.UEs, sat_snr_dict_real, result["allocation"])
        ...
        observation = {}
        distance_dict = get_info_by_time(self.visibles_t_u, self.current_slot, self.common_sats, self.ue_num)
        sat_app_elevation_angles = {}
        for agent_id in self.agent_ids:
            sat_id = int(agent_id)
            app_elevation_angles = []
        ...
        for sat in self.leo_sats:
            ...
            self._observation_space[str(sat_id)] = {}
            self._observation_space[str(sat_id)]["base_features"] = np.array([window_progress, traffic_intensity, rb_ratio, power_ratio], dtype=np.float32)
            # 其次是当前每个app相对当前卫星的仰角
            self._observation_space[str(sat_id)]['ele_angles'] = np.array(sat_app_elevation_angles[sat_id], dtype=np.float32)
            # 接着是每个app的7个基本特征，当前队列长度，到达率估计，时延/带宽满意度，该app分配的带宽比例，带宽/时延需求
            app_features = []
            current_queue_lengths = []
            arrival_rate_estimates = []
            ...
            # 然后是当前卫星的snr空间，拆分成10份（对应0.1s -> 0.01s的时间粒度）
            snr_spaces = {}
            sat_snr_info = sat_snr_dict_real[sat.ID]
            for j in range(10):
                if f"snr_{j}" not in snr_spaces:
                    # 用用户数 * RB数来初始化
                    snr_spaces[f"snr_{j}"] = []
    ...
    def return_zero_obs(self):
        observation = {}
        sat_app_elevation_angles = {}
        distance_dict = get_info_by_time(self.visibles_t_u, self.current_slot, self.common_sats, self.ue_num)  # 获取第0个时隙的用户-卫星距离信息
        for agent_id in self.agent_ids:
            sat_id = int(agent_id)
            app_elevation_angles = []
            for ue_id, ue in self.UEs.items():
